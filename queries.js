...
const getUserById = (request, response) => {
  ...
  console.log(id)
  pool.query('select DATE(created_at) date,count(tripid) trips  from gpslog where deviceid =$1 group by  DATE(created_at) , tripid order by  DATE(created_at) desc', [id], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
